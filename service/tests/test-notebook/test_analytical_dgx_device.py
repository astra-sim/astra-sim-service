def test_analytical_dgx_device(port_number):
    ...
    try:
        ...
        astra = AstraSim(f"0.0.0.0:{port_number}", tag="analytical_dgx_device")
        ...
        # ##### Set ASTRA-sim network backend to ANALYTICAL_CONGESTION_AWARE

        astra.configuration.network_backend.analytical_congestion_aware.topology.choice = (
            astra.configuration.network_backend.analytical_congestion_aware.topology.INFRAGRAPH
        )

        # ##### Adding ASTRA-sim - Infragraph specific annotation

        node = astra.configuration.infragraph.annotations.nodes.add(server.name)
